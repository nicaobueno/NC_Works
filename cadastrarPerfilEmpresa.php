<?php
    ...
    if(isset($_POST["enviarCad"])){
        ...
        if(empty($nomeEmpresa) || empty($CNPJ) || empty($CEP) || empty($pais) || empty($ufs) || empty($cidade) || empty($bairro)  || empty($rua) || empty($email)){
            ...
            $insert = "INSERT INTO tb_empresa VALUES (null, '$nomeEmpresa','$CNPJ','$CEP','$pais','$ufs','$cidade','$bairro','$rua','$email','$site_Empresa','$contato','$sobre')";
            $salvar = mysqli_query($link, $insert);    
                if ($salvar) {
                    ...
                    echo "<meta http-equiv='refresh' content='0;url=index.php?action=perfil' />";
                }
        }
    }
    printf($_POST);
